...
def find_lane_pixels_using_prev_poly(binary_warped):
    ...
    right_lane_inds = ((nonzerox > (prev_right_fit[0]*(nonzeroy**2) + prev_right_fit[1]*nonzeroy + 
     ...
     # ตรวจสอบว่าพบพิกเซลของเส้นทางสำหรับเส้นซ้ายและเส้นขวาหรือไม่
#---------------------------------------------------------------------------------------------------------------------
    if len(left_lane_inds) == 0 or len(right_lane_inds) == 0:
        # หากไม่พบพิกเซลของเส้นทางใช้วิธีการค้นหาโดยใช้ฮิสโตแกรมเป็นพื้นฐาน
        left_lane_inds, right_lane_inds = find_lane_pixels_using_histogram(binary_warped)         
#---------------------------------------------------------------------------------------------------------------------                                                                  
